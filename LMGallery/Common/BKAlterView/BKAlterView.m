...
@interface BKAlterView()
-(void)hideMessage;
@end

@implementation BKAlterView
...
//    BOOL ish = [[UIApplication sharedApplication] isStatusBarHidden];
    BKAlterView *alt = [[BKAlterView alloc] initWithFrame:CGRectMake(0, 0, inview.bounds.size.width,inview.bounds.size.height) andType:type withMsg:msg offsetY:offsetY];
...
+(void)showInfoMsg:(NSString *)msg inview:(UIView*)inview offsetY:(float)offsetY{
    [BKAlterView showMsg:msg type:BKMsgInfo inview:inview offsetY:offsetY];
}
+(void)showErrorMsg:(NSString *)msg inview:(UIView*)inview offsetY:(float)offsetY{
    [BKAlterView showMsg:msg type:BKMsgError inview:inview offsetY:offsetY];
}
...
- (id)initWithFrame:(CGRect)frame andType:(BKMsg)type withMsg:(NSString *)msg offsetY:(float)offsetY
{
    ...
    if (self) {
        ...
        tank.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.8];
        
        NSString *imgname = @"ATick";
        if (type==BKMsgError) {
            imgname = @"Error";
        }
        UIImage *img = [UIImage imageNamed:imgname];
        UIImageView *imgview = [[UIImageView alloc] initWithImage:img];
        imgview.frame = CGRectMake((tw - img.size.width)/2,18, img.size.width, img.size.height);
        ...
        [self performSelector:@selector(hideMessage) withObject:nil afterDelay:1];
    }
    return self;
}
- (void)dealloc {
